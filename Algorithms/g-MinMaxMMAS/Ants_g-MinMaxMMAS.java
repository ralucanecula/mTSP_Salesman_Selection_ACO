...
public class Ants {
    ...
    static boolean mmas_flag; /* MAX-MIN ant system (MMAS) */
    ...
    /**simulation of the pheromone trail evaporation for MMAS
     * pheromones are reduced by factor rho
     * if local search is used, this evaporation procedure, only considers links between a city and those cities of its candidate list
     */
    static void mmas_evaporation_nn_list() {
		...
		for (i = 0; i < (MTsp.n + 1); i++) {
		    for (j = 0; j < nn_ants; j++) {
				help_city = MTsp.instance.nn_list[i][j]; 
    ...
    /**simulation of the pheromone trail evaporation
     * if local search is used, this evaporation procedure only considers links between a city and those cities of its candidate list
     */
    ...
    //calculates heuristic info times pheromone for arcs in nn_list
    static void compute_nn_list_total_information() {
		int i, j, h;
	
		for (i = 0; i < (MTsp.n + 1); i++) {
		    for (j = 0; j < nn_ants; j++) {
				h = MTsp.instance.nn_list[i][j];
    ...
    /**only for MMAS without local search; keeps pheromone trails inside trail limits; pheromones are forced to interval [trail_min,trail_max]
     * 
     */
