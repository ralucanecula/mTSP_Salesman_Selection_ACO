...
public class TSP_ACO {
    ...
    public static boolean ls_flag = false; 
    ...
    static void init_try() {
		 ...
		 */
		if (!(Ants.acs_flag || Ants.mmas_flag)) {
		...
		if (Ants.mmas_flag) {
		    ...
		    Ants.trail_min = Ants.trail_max / (2. * (MTsp.n + 1));
    ...
    static void update_statistics(boolean saveIterCosts) {
		...
		if (a.longest_tour_length < Ants.best_so_far_ant.longest_tour_length) {
		    ...
		    Ants.copy_from_to(a, Ants.restart_best_ant);
	
		    InOut.found_best = InOut.iteration;	 
		    InOut.restart_found_best = InOut.iteration;
		    if (Ants.mmas_flag) {
		    	if (!ls_flag) {
				    p_x = Math.exp(Math.log(0.05) / (MTsp.n + 1));
				    Ants.trail_min = 1. * (1. - p_x) / (p_x * (double) ((Ants.nn_ants + 1) / 2));
				    Ants.trail_max = 1. / ((Ants.rho) * Ants.best_so_far_ant.longest_tour_length);
		...
		if (a.longest_tour_length < Ants.restart_best_ant.longest_tour_length) {
		    Ants.copy_from_to(a, Ants.restart_best_ant);
		    
		    InOut.restart_found_best = InOut.iteration;
		    //System.out.println("Iter: " + InOut.iteration + " Restart best ant -> longest tour=" + Ants.restart_best_ant.longest_tour_length);
		}
		...
		if (Ants.ants[iteration_best_ant].longest_tour_length < Ants.restart_best_ant.longest_tour_length) {
		    Ants.copy_from_to(Ants.ants[iteration_best_ant], Ants.restart_best_ant);
   ...
   //at every 5 iterations save the value of the longest cost of the solution/tour given by the best so far ant
    static void search_control_and_statistics(ArrayList<Double> iterLongestCost, ArrayList<Integer> iterNumber, boolean saveDetailedOutput, int trial)
    {
    	...
    	if ((InOut.iteration % 5) == 0) {
    	//if ((InOut.iteration % 10) == 0) {
    	    InOut.branching_factor = InOut.node_branching(InOut.lambda);
    	    //System.out.println("Iter: " + InOut.iteration + " Best ant -> longest tour=" + Ants.best_so_far_ant.longest_tour_length + ", b_fac " + InOut.branching_factor);

    	    //if (InOut.iteration - InOut.restart_found_best >= 100) {
    	    //if ((InOut.branching_factor < InOut.branch_fac) && (InOut.iteration - InOut.restart_found_best > 250)) {
    	    if (((InOut.branching_factor <= InOut.branch_fac) && (InOut.iteration - InOut.restart_found_best > 100)) || ((InOut.iteration - InOut.restart_found_best >= 200))) {
    			// Ants.pheromone trail re-initialisation takes place
    			System.out.println("Reinitialisation of pheromone trails...");
    			Ants.restart_best_ant.longest_tour_length = Integer.MAX_VALUE;
    			if (Ants.acs_flag) {
    				initTrail = 1. / ((double) (MTsp.n + 1) * Ants.best_so_far_ant.longest_tour_length);
    ...
    /** manage global Ants.pheromone deposit for MAX-MIN Ant System
     * either the iteration-best or the best-so-far ant deposit Ants.pheromone on matrix "Ants.pheromone"
     */
    static void mmas_update() {
		...
		if (InOut.iteration % Ants.u_gb == 0) {
		    ...
		    Ants.global_update_pheromone(Ants.ants[iteration_best_ant]);
		} else {
	...
	static void pheromone_trail_update()
	{
		...
		if (Ants.as_flag || Ants.mmas_flag) {
			if (ls_flag) {
				if (Ants.mmas_flag)
		 ...
		 */
		if (Ants.mmas_flag && ls_flag == false)
	...
	public static void main(String[] args) {
		...
		for (int trial = 0; trial < 30; trial++) {
			...
			if (trial == 0) {
				System.out.println("Number of used ants: " + Ants.n_ants);
			}
