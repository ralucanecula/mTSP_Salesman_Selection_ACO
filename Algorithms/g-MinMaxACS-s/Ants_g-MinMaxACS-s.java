...
public class Ants {
    ...
    static class Ant {
		...
		int indexLongestTour;  //the index of the longest subtour
    ...
    static void allocate_ants() {
	    ...
	    restart_best_ant.tours = (ArrayList<Integer>[])new ArrayList[MTsp.m];
	    restart_best_ant.tour_lengths = new double[MTsp.m];
	    for (j = 0; j < MTsp.m; j++) {
	    	restart_best_ant.tours[j] = new ArrayList<Integer>();
	    	restart_best_ant.tour_lengths[j] = 0;
	    }
	    restart_best_ant.visited = new boolean[MTsp.n];
	    restart_best_ant.toVisit = MTsp.n;
		...
		prob_of_selection = new double[MTsp.m][nn_ants];
		for (j = 0; j < MTsp.m; j++) {
			for (i = 0; i < nn_ants; i++) {
   ...
   /* static void compute_total_information()
    ...
    static int[] choose_best_next(Ant a)
    {
		int city, current_city, next_city, salesman = 0;
		double value_best = -1.;  /* values in total matrix are always >= 0.0 */
		double help, partialTourLength;
		int[] values = new int[2];
		
		next_city = MTsp.n;
		
		for (int indexSalesman = 0; indexSalesman < MTsp.m; indexSalesman++) {
	    	...
	    	for (city = 0; city < MTsp.n; city++) {
				...
				; /* city already visited, do nothing */
			    	...
			    	partialTourLength = Tsp.compute_tour_length_(a.tours[indexSalesman]);
			    	//when there is an empty subtour, with no visited cities, make such that
			    	//partialTourLength doesn't count in the initial stages of the algorithm as
			    	//heuristic information
			    	if (partialTourLength == 0) {
			    		partialTourLength = 1;
			    	}
					help = HEURISTIC(current_city, city + 1) * (1.0 / partialTourLength);
		...
		a.tours[salesman].add(next_city);
    ...
    static int[] neighbour_choose_best_next(Ant a) {
		int i, current_city, next_city, help_city, salesman = 0;
		double value_best = -1;  //values in total matrix are always >= 0.0 
		double help, partialTourLength;
		int[] values = new int[2];
		...
		for (int indexSalesman = 0; indexSalesman < MTsp.m; indexSalesman++) {
			...
			for (i = 0; i < nn_ants; i++) {
			    ...
			    if (a.visited[help_city - 1])
					...
					help = HEURISTIC(current_city, help_city) * (1.0 / partialTourLength);
					help = Math.pow(help, beta);
    ...
    static int[] neighbour_choose_and_move_to_next(Ant a) {
		int i, help, city, salesman = 0;
		int current_city = 0;
		double rnd, partial_sum = 0., sum_prob = 0.0;
		double prob_ptr[][];
		double help1, partialTourLength;
		int[] values = new int[2];
		int[] tempCities = new int[MTsp.m]; 
		...
		if ((q_0 > 0.0) && (Utilities.random01() < q_0)) {
		     ...
		     */
		    values = neighbour_choose_best_next(a);
		    return values; 
		}
		...
		for (int indexSalesman = 0; indexSalesman < MTsp.m; indexSalesman++) {
			...
			for (i = 0; i < nn_ants; i++) {
			    ...
			    if (a.visited[city - 1])
					...
					help1 = HEURISTIC(current_city, city) * (1.0 / partialTourLength);
		...
		if (sum_prob <= 0.0) {
		    ...
		    partial_sum = 0;
		    for (int indexSalesman = 0; indexSalesman < MTsp.m && !done; indexSalesman++) {
		    	i = 0;
			    ...
			    while (partial_sum <= rnd) {
					...
					if (indexSalesman == (MTsp.m - 1) && i >= prob_ptr[indexSalesman].length && partial_sum <= rnd) {
    ...
    static double nn_tour() {
		...
		for (int i = 0; i < MTsp.m; i++) {
			...
			ants[0].tour_lengths[i] = Tsp.compute_tour_length(ants[0].tours[i]);
		...
		ants[0].indexLongestTour = idLongestTour;
		
		if (TSP_ACO.ls_flag) {
			ants[0] = TSP_ACO.local_search(ants[0]);
			
			//compute new distances and update longest tour
			for (int l = 0; l < MTsp.m; l++) {
				ants[0].tour_lengths[l] = Tsp.compute_tour_length(ants[0].tours[l]);
				sum += ants[0].tour_lengths[l];
				if (longestTourLength < ants[0].tour_lengths[l]) {
					longestTourLength = ants[0].tour_lengths[l];
					idLongestTour = l;
				}
			}
