...
public class TSP_ACO {
    ...
    //choose in a deterministic way (according to the length of its subtour) the next salesman 
    //that will visit a city; when all the subtours are empty choose in a random manner the salesman
    static int chooseSalesmanDeterministic(Ant a) {
    	...
    	if (isEmpty) {
    		...
    		for (int i = 1; i < MTsp.m; i++) {
    			if (a.tour_lengths[i] < min) {
    				min = a.tour_lengths[i];
    				indexMin = i;
    		    }
    		}
    		nextSalesman = indexMin;
    	}
    ...
    static void construct_solutions() {
		...
		while (!isDone()) {
		    for (k = 0; k < Ants.n_ants; k++) {
		    	if (Ants.ants[k].toVisit > 0) {
		    		//choose for each ant in a deterministic way the salesman that will visit the next city
		    		//the salesman is selected in a deterministic way according to the length of its subtour: 
		    		//always it is selected the salesman which has the smallest value for its subtour
		    		
		    		//compute lengths for all partial subtours constructed so far 
			    	for (int i1 = 0; i1 < MTsp.m; i1++) {
			    		Ants.ants[k].tour_lengths[i1] = Tsp.compute_tour_length_(Ants.ants[k].tours[i1]);
			    	}
		    		salesman = chooseSalesmanDeterministic(Ants.ants[k]); 
